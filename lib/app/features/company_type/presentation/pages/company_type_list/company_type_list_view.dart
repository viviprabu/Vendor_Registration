...
import 'package:vendor_registration/app/features/company_type/presentation/pages/company_type_list/edit_company_type_popup.dart';
...
import 'add_company_type_popup.dart';

class CompanyTypeListView extends StatefulWidget {
  const CompanyTypeListView({super.key});

  @override
  _CompanyTypeListViewState createState() => _CompanyTypeListViewState();
}

class _CompanyTypeListViewState extends State<CompanyTypeListView> {
  ///_____________________________________________________________________Variables_______________________________
  late List<CompanyType> _filteredData;
  late List<CompanyType> paginatedData=[];
  ...
  void initState() {
    super.initState();
    _filteredData = List.from(companyType);
  }
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return BackdropFilter(
          ...
          child: const AddCompanyTypeDialog(),
        );
      },
    );
  }
  ...
  void _showEditFormDialog(CompanyType companyTypeData) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return BackdropFilter(
  ...
  Widget build(BuildContext context) {
    context.read<CompanyTypeBloc>().add(CompanyTypesListEvent());
    ...
    return BlocBuilder<CompanyTypeBloc, CompanyTypeState>(
      builder: (context, state) {
        if (state is CompanyTypesListState) {
          companyType = state.companyTypes;

          if (_searchQuery.isNotEmpty) {
            _filteredData = companyType;
            // .where((data) =>
                        ...
                        isMobile || isTablet
                                  ...
                                  children: [
                                    SingleChildScrollView(
                                      ...
                                      child: ConstrainedBox(
                                        ...
                                        child: companyTypeListDataTable(
                                    ...
                                    Padding(
                                      padding: sizeInfo.padding,
                                      child: Text(
                                        '${l.S.of(context).showing} ${_currentPage * _rowsPerPage + 1} ${l.S.of(context).to} ${_currentPage * _rowsPerPage + companyType.length} ${l.S.of(context).OF} ${_filteredData.length} ${l.S.of(context).entries}',
                            ...
                            : SingleChildScrollView(
                                ...
                                child: ConstrainedBox(
                                  ...
                                  child:
                                      companyTypeListDataTable(context, paginatedData),
                                ),
                              ),
  ...
  Theme companyTypeListDataTable(BuildContext context, List<CompanyType> companyType) {
    final lang = l.S.of(context);
    final theme = Theme.of(context);
                      ...
                      Checkbox(
                        ...
                        onChanged: (selected) {
                          setState(() {
                            ...
                            _selectAll = companyType.every((d) => d.isSelected);
                          });
                        },
                      ),
                      const SizedBox(width: 12.0),
